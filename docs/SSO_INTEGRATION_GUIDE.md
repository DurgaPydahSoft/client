...
## Portal Backend Configuration: SSO Verify URL

The portal (e.g. HMS) verifies SSO tokens **only by calling the CRM backend** — no local JWT verification. Configuration is via environment variables on the **portal backend** (the app that exposes `/api/auth/verify-token` and calls the CRM to validate the token).

### Environment variable: `SSO_VERIFY_URL` (required for SSO)

- **Value:** The CRM (or other auth) backend **base URL only** — do **not** include the path.
  - Examples: `http://localhost:8000`, `https://crm.example.com`
  - Trailing slashes are optional; they are normalized (e.g. `http://localhost:8000/` is fine).
- **Behaviour:** The portal backend calls **`{SSO_VERIFY_URL}/auth/verify-token`**. The path `/auth/verify-token` is appended automatically.
  - Example: with `SSO_VERIFY_URL=http://localhost:8000`, the portal calls `http://localhost:8000/auth/verify-token` with `POST` and body `{ "encryptedToken": "..." }`.
- **Required:** SSO login will not work unless `SSO_VERIFY_URL` is set. The portal does not verify the token locally (no JWT secret is used for the incoming SSO token).

### Summary

| Variable         | Required | Purpose |
|------------------|----------|--------|
| **SSO_VERIFY_URL** | Yes (for SSO) | CRM backend **base URL**. Portal calls `{SSO_VERIFY_URL}/auth/verify-token` to verify the token. |
| **JWT_SECRET**     | Yes (for auth) | Portal’s own JWT secret; used to sign tokens issued after successful SSO verification. |

### Example (.env on portal backend)

```env
# CRM backend base URL only (portal will call /auth/verify-token on this host)
SSO_VERIFY_URL=http://localhost:8000
```
