...
const ElectricityBills = () => {
  ...
  return (
    <div>
      ...
      {/* Toolbar: hostel badge + category + rooms count */}
      <div className="-mx-4 px-4 py-2.5 mb-3 bg-white border-b border-gray-100 sm:mx-0 sm:px-0 sm:py-0 sm:bg-transparent sm:border-0 sm:mb-4 flex items-center gap-2">
        ...
        <select
          ...
          className="flex-1 min-w-0 max-w-[11rem] sm:max-w-xs px-2 py-1.5 text-xs sm:text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 disabled:bg-gray-100"
        >
        ...
        <span className="ml-auto text-xs sm:text-sm font-medium text-gray-600 flex-shrink-0">
          Rooms: <span className="font-semibold text-gray-900">{rooms.length}</span>
        </span>
      </div>

      {/* Bulk Billing */}
      <div className="-mx-4 px-4 py-3 bg-white sm:mx-0 sm:rounded-xl sm:shadow-sm sm:p-4">
        {/* Global Controls - Sticky */}
        <div className="sticky top-14 sm:static z-10 mb-4 sm:mb-6 p-3 sm:p-4 bg-gray-50 rounded-lg shadow-sm border border-gray-200 space-y-2.5 sm:space-y-3">
          <div className="grid grid-cols-2 gap-2 sm:gap-4">
            <div>
              <label className="block text-[10px] sm:text-sm font-medium text-gray-700 mb-1">Billing Month</label>
              <input 
                type="month" 
                value={bulkMonth}
            ...
            <div>
              <label className="block text-[10px] sm:text-sm font-medium text-gray-700 mb-1">Default Rate/Unit</label>
              <input 
                type="number" 
                placeholder="e.g., 5"
                value={bulkRate}
        ...
        <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-3">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-amber-800">Generator Bill (Hostel)</p>
              <p className="mt-1 text-xs text-amber-700">
                Diesel litres × ₹/litre = hostel total, then shared by attendance (same rules as electricity).
              </p>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] gap-2 sm:min-w-[420px]">
              <input
                type="number"
                min="0"
                step="0.01"
                    ...
                    <tr key={bill.roomId} className={`${isLockedSavedBill ? 'bg-green-100' : (bill.isEdited || isEditingSavedBill ? 'bg-blue-50' : '')}`}>
                      <td className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm font-medium text-gray-900">
                        <div className="flex flex-col">
                          <button
                            type="button"
                            onClick={() => handleOpenRoomOccupants(bill.roomId, isValid ? total : null)}
                      ...
                      <td className="px-2 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm">
                        <span className={`font-semibold ${!isValid && !isLockedSavedBill ? 'text-red-500' : 'text-green-600'}`}>
                ...
                >
                  ...
                  <div className="space-y-3">
                    {isDualMeter ? (
                      <>
                        <div className="flex items-center gap-2">
                          <span className="inline-flex items-center px-2 py-1 rounded-lg bg-blue-50 border border-blue-200 text-[10px] font-semibold text-blue-800 flex-shrink-0">
                            Dual Meter
                          </span>
                          <div className="flex items-center gap-1.5 flex-1 min-w-0">
                            <label className="text-[10px] font-medium text-gray-600 whitespace-nowrap">Rate</label>
                            <input
                              type="number"
                              placeholder={bulkRate || 'Rate'}
                              value={isLockedSavedBill ? rate : bill.rate}
                              onChange={(e) => handleBulkBillChange(bill.roomId, 'rate', e.target.value)}
                              disabled={isLockedSavedBill}
                              className="w-full min-w-0 px-2 py-1 text-sm border border-gray-300 rounded-lg disabled:bg-gray-200 focus:ring-2 focus:ring-green-500 focus:border-green-500"
                            />
                          </div>
                        </div>

                        {/* Meter 1 — Start / End same row */}
                        <div className="border-l-4 border-blue-500 pl-2">
                          <p className="text-[10px] font-semibold text-gray-700 mb-1.5">Meter 1</p>
                          <div className="grid grid-cols-2 gap-2">
                            <div>
                              <label className="block text-[10px] font-medium text-gray-600 mb-0.5">Start</label>
                              <input
                                type="number"
                                value={isLockedSavedBill ? meter1StartUnits : bill.meter1StartUnits}
                                onChange={(e) => handleBulkBillChange(bill.roomId, 'meter1StartUnits', e.target.value)}
                                disabled={isLockedSavedBill}
                                className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded-lg disabled:bg-gray-200 focus:ring-2 focus:ring-green-500 focus:border-green-500"
                                placeholder="Start"
                              />
                            </div>
                            <div>
                              <label className="block text-[10px] font-medium text-gray-600 mb-0.5">End</label>
                              <input
                                type="number"
                                placeholder="End"
                                value={isLockedSavedBill ? meter1EndUnits : bill.meter1EndUnits}
                                onChange={(e) => handleBulkBillChange(bill.roomId, 'meter1EndUnits', e.target.value)}
                                disabled={isLockedSavedBill}
                                className={`w-full px-2 py-1.5 text-sm border rounded-lg disabled:bg-gray-200 focus:ring-2 focus:ring-green-500 focus:border-green-500 ${
                        ...
                        {/* Meter 2 — Start / End same row */}
                        <div className="border-l-4 border-green-500 pl-2">
                          <p className="text-[10px] font-semibold text-gray-700 mb-1.5">Meter 2</p>
                          <div className="grid grid-cols-2 gap-2">
                            <div>
                              <label className="block text-[10px] font-medium text-gray-600 mb-0.5">Start</label>
                              <input
                        ...
                        {!isValid && !isLockedSavedBill && (
                          <p className="text-xs text-red-600">End units must be ≥ start units for both meters</p>
                        )}
                      </>
                    ) : (
                      <>
                        <div className="flex items-center gap-2">
                          <span className="inline-flex items-center px-2 py-1 rounded-lg bg-gray-50 border border-gray-200 text-[10px] font-semibold text-gray-700 flex-shrink-0">
                            Single Meter
                          </span>
                          <div className="flex items-center gap-1.5 flex-1 min-w-0">
                            <label className="text-[10px] font-medium text-gray-600 whitespace-nowrap">Rate</label>
                        ...
                        {/* Single Meter — Start / End same row */}
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            <label className="block text-[10px] font-medium text-gray-600 mb-0.5">Start</label>
                            <input
                        ...
                        <div className="text-center">
                          ...
                          {isValid && (
                            <button
                              type="button"
                              onClick={() => handleOpenRoomOccupants(bill.roomId, total)}
                              className="text-[10px] text-blue-600 hover:text-blue-800 hover:underline font-medium mt-0.5"
                            >
                              Preview Shares
                            </button>
                          )}
      ...
      {/* Preview Shares modal — relevant info only */}
      {occupantsModal.open && (
        <div
        ...
        >
          <div
            className="w-full max-w-lg rounded-t-xl sm:rounded-lg bg-white shadow-xl max-h-[85vh] flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between border-b border-gray-200 px-4 py-3 flex-shrink-0">
              <div>
                <h3 className="text-base font-semibold text-gray-900">
                  Room {occupantsModal.roomNumber} — Shares
                </h3>
                <p className="text-xs text-gray-500 mt-0.5">
                  {occupantsModal.eligibleCount ?? 0} student{(occupantsModal.eligibleCount ?? 0) === 1 ? '' : 's'}
                  {occupantsModal.previewTotalAmount !== null && (
                    <span className="ml-1 font-semibold text-green-700">
                      · ₹{occupantsModal.previewTotalAmount.toFixed(2)}
                    </span>
                  )}
                </p>
              </div>
              ...
              {occupantsModal.loading ? (
                ...
                (() => {
                  ...
                  const hasNewJoiningInModal = occupantsModal.students.some((s) => {
                    const days = s.billingDays ?? totalDaysInMonth;
                  ...
                  const totalBillingDays = eligibleStudents.reduce(
                    (sum, s) => sum + (s.billingDays ?? totalDaysInMonth),
                    0
                  );
                  ...
                  if (eligibleStudents.length === 0) {
                    return (
                      <p className="py-6 text-center text-sm text-gray-500">
                        No students to bill for this month.
                      </p>
                    );
                  }
                      ...
                      {occupantsModal.previewTotalAmount !== null && (
                        <p className="mb-3 text-[11px] text-gray-500">
                          {hasNewJoiningInModal
                            ? 'Split by billing days (mid-month joiners/leavers pay for occupied days).'
                            : 'Equal split among occupants.'}
                        </p>
                      )}
                      <ul className="divide-y divide-gray-100">
                        {eligibleStudents.map((s) => {
                          const billingDays = s.billingDays ?? totalDaysInMonth;
                          ...
                          const totalShare = calculatedShare + (s.generatorShare || 0);

                          const joinedDateObj = s.joinedDate ? new Date(s.joinedDate) : null;
                          const hasJoinedMidMonth = joinedDateObj && joinedDateObj >= monthStart && joinedDateObj <= monthEnd;

                          const leftDateObj = s.leftDate ? new Date(s.leftDate) : null;
                          const hasLeftMidMonth = leftDateObj && leftDateObj >= monthStart && leftDateObj <= monthEnd;

                          return (
                            <li key={s._id || s.hostelRequestId || s.admissionNumber} className="py-3">
                              <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0">
                                  ...
                                  {/* Stay / Join / Left Dates Info */}
                                  <div className="mt-1 space-y-0.5">
                                    <p className="text-[10px] text-gray-400">
                                      Billed duration: <span className="font-medium text-gray-600">{billingDays} days</span>
                                    </p>
                                    {hasJoinedMidMonth && (
                                      <p className="text-[10px] text-blue-600 font-medium">
                                        Joined: {joinedDateObj.toLocaleDateString('en-IN')}
                                      </p>
                                    )}
                                    {hasLeftMidMonth && (
                                      <p className="text-[10px] text-red-500 font-medium">
                                        Vacated (NOC): {leftDateObj.toLocaleDateString('en-IN')}
                                      </p>
                                    )}
                                  </div>
                                </div>
                                  ...
                                  {occupantsModal.previewTotalAmount !== null ? (
                                    <div className="space-y-0.5">
                                      <div className="text-xs text-gray-500">
                                        Elec: ₹{calculatedShare}
                                      </div>
                                      {s.generatorShare > 0 && (
                                        <div className="text-xs text-gray-500">
                                          Gen: ₹{s.generatorShare}
                                        </div>
                                      )}
            ...
            <div className="border-t border-gray-200 px-4 py-3 text-right flex-shrink-0">
              <button
                type="button"
                onClick={closeOccupantsModal}
