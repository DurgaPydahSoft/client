...
export const normalizeStudentIdentityKey = (student) => {
  const admission = (student?.admissionNumber || '').toString().trim().toUpperCase();
...
export const scoreCanonicalStudent = (student) => {
 ...
 */
export const dedupeStudentsByIdentity = (students) => {
  ...
  for (const student of students) {
    ...
    if (!existing || scoreCanonicalStudent(student) > scoreCanonicalStudent(existing)) {
